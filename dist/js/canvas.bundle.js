...
var backgroundMusic = new Audio('./audio/alternate.mp3');
...
function init() {
    ...
    player = new _models_player__WEBPACK_IMPORTED_MODULE_1__["Player"](canvas, 10, 'ivory');
    projectiles = [];
    particles = [];
    enemies = [];
    powerUps = [];
    ...
    for (var i = spacing / 2; i < canvas.width; i += spacing) {
        for (var j = spacing / 2; j < canvas.height; j += spacing) {
        ...
        projectiles.forEach(function (projectile, projectileIndex) {
            ...
            if (dist - enemy.radius - projectile.radius < 0.1 && enemy.radius > 0) {
                ...
                if (enemy.hit(projectile.power)) {
                    ...
                    var splashAmount_1 = Math.random() * 18 + 6;
...
function addScore(basePoints, projectile) {
    var multiplier = 1 + (combo > 5 ? combo / 10 : 0);
