...
var currentSong = inCloudsSongURL;
...
function init() {
    ...
    player = new _models_player__WEBPACK_IMPORTED_MODULE_1__["Player"](topLeft, bottomRight, 'ivory', keys);
    projectiles = [];
    particles = [];
    enemies = [];
    powerUps = [];
...
function updateEnemies() {
    enemies.forEach(function (enemy, index) {
        ...
        // bounce off walls
        var collidedWithX = enemy.center.x - enemy.radius + enemy.velocity.x < topLeft.x || enemy.center.x + enemy.radius + enemy.velocity.x > bottomRight.x;
        var collidedWithY = enemy.center.y - enemy.radius + enemy.velocity.y < topLeft.y || enemy.center.y + enemy.radius + enemy.velocity.y > bottomRight.y;
        if (enemy.inPlay) {
            if (collidedWithX) {
        ...
        for (var i = index + 1; i < enemies.length; i++) {
            var e = enemies[i];
            if (enemy.distanceBetween(e) < 0.1)
...
function updatePowerups() {
    powerUps.forEach(function (powerUp, index) {
        var dist = Math.hypot(player.center.x - powerUp.x, player.center.y - powerUp.y);
...
function lightUpBackgroundParticles() {
    backgroundParticles.forEach(function (bp) {
        var dist = Math.hypot(player.center.x - bp.x, player.center.y - bp.y);
        var hideRadius = 125;
        if (dist < hideRadius) {
...
function spawnEnemy(level) {
    enemies.push(new _models_enemies__WEBPACK_IMPORTED_MODULE_2__["Enemy"](outOfBounds(), player.center, level));
}
function spawnBoss() {
    scene.boss = true;
...
function cleanup() {
    ...
    projectiles.forEach(function (projectile, index) {
        projectile.update(c);
        if (projectile.center.x + projectile.radius < 0 ||
...
function hitSplash(p, color, amount, angle) {
    ...
    for (var i = 0; i < amount; i++) {
        particles.push(new _models_particles__WEBPACK_IMPORTED_MODULE_4__["Particle"](p.x, p.y, Math.random() * 2, color, new _models_base__WEBPACK_IMPORTED_MODULE_5__["Velocity"]((Math.random() - 0.5) + xBias, (Math.random() - 0.5) + yBias)));
    }
}
...
function outOfBounds(radius) {
    if (radius === void 0) { radius = 100; }
    // assume radius since it should be big enough
    if (Math.random() < 0.5) {
        var x = Math.random() < 0.5 ? 0 - radius : canvas.width + radius, y = Math.random() * canvas.height;
        return new _models_base__WEBPACK_IMPORTED_MODULE_5__["Point"](x, y);
    }
    else {
        var x = Math.random() * canvas.height, y = Math.random() < 0.5 ? 0 - radius : canvas.height + radius;
        return new _models_base__WEBPACK_IMPORTED_MODULE_5__["Point"](x, y);
    }
}
...
/*!*******************************!*\
  !*** ./src/js/models/base.ts ***!
  \*******************************/
/*! exports provided: Circle, Point, Velocity */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var Circle = /** @class */ (function () {
    function Circle(center, radius, color, friction) {
        if (friction === void 0) { friction = 0.997; }
        this.center = center;
        this.radius = radius;
        this.color = color;
...
var Velocity = /** @class */ (function () {
    ...
    Velocity.prototype.speed = function () {
...
var Enemy = /** @class */ (function (_super) {
    ...
    function Enemy(spawnPoint, target, level) {
        ...
        _this = _super.call(this, spawnPoint, radius + level * 10, randomColor()) || this;
        _this.points = 200 + level * 50;
        _this.color = randomColor();
        _this.baseSpeed = 0.75 + (level * 0.15) + (Math.random() * 0.15);
        _this.inPlay = false;
        var angle = _this.center.angleTo(target);
        _this.velocity = new _base__WEBPACK_IMPORTED_MODULE_1__["Velocity"](Math.cos(angle) * _this.baseSpeed * 8, Math.sin(angle) * _this.baseSpeed * 8);
        return _this;
    }
...
var Projectile = /** @class */ (function (_super) {
    ...
    function Projectile(x, y, radius, color, velocity, power) {
        var _this = _super.call(this, new _base__WEBPACK_IMPORTED_MODULE_0__["Point"](x, y), radius, color) || this;
...
var Player = /** @class */ (function (_super) {
    ...
    function Player(topLeft, bottomRight, color, keys) {
        var _this = _super.call(this, new _base__WEBPACK_IMPORTED_MODULE_1__["Point"]((topLeft.x + bottomRight.x) / 2, (topLeft.y + bottomRight.y) / 2), playerRadius, color) || this;
    ...
    Player.prototype.shoot = function (_a) {
        var x = _a.x, y = _a.y;
        var target = new _base__WEBPACK_IMPORTED_MODULE_1__["Point"](x, y);
        var angle = this.center.angleTo(target);
        var velocity = new _base__WEBPACK_IMPORTED_MODULE_1__["Velocity"](Math.cos(angle) * this.shotSpeed, Math.sin(angle) * this.shotSpeed);
        var s = shootAudio.cloneNode();
        s.volume = 0.5;
        s.play();
        return new _particles__WEBPACK_IMPORTED_MODULE_0__["Projectile"](this.center.x, this.center.y, 5, this.color, velocity, this.power);
    };
    Player.prototype.update = function (c) {
        ...
        this.velocity.x *= this.friction;
        this.velocity.y *= this.friction;
        if (this.center.x - this.radius + this.velocity.x > this.topLeft.x &&
        ...
        if (this.center.y - this.radius + this.velocity.y > this.topLeft.y &&
...
var PowerUp = /** @class */ (function () {
    function PowerUp(_a) {
        ...
        this.velocity = new _base__WEBPACK_IMPORTED_MODULE_0__["Velocity"](Math.cos(angle) + Math.random(), Math.sin(angle) + Math.random());
        this.width = 14;
        this.height = 19;
        this.inPlay = false;
    }
